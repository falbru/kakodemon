...
void InlineMenuView::init(domain::Renderer *renderer, KakouneContentView *kakoune_content_view)
{
    m_renderer = renderer;
                            ...
                            const domain::Rectangle &content_bounds)
{
    ...
    m_width = std::min(menu_item_width + BORDER_THICKNESS * 2.0f + SPACING_MEDIUM * 2.0f +
                           m_scrolled_menu_items->getRightPadding(menu.getItems().items.size()),
                       std::min(MAX_MENU_WIDTH, render_context.screen_width));
...
domain::MouseMoveResult InlineMenuView::handleMouseMove(float x, float y, const domain::Menu &menu)
{
    if (menu.hasItems())
    {
...
std::optional<int> InlineMenuView::findItemAtPosition(float x, float y, const MenuViewState &state,
...
bool InlineMenuView::handleMouseButton(domain::MouseButtonEvent event, MenuViewState &state, const domain::Menu &menu)
{
...
void InlineMenuView::handleMouseScroll(MenuViewState &state, int scroll_amount, const domain::Menu &menu)
{
    if (!menu.hasItems())
        return;
