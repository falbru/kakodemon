...
void PaneBorderView::render(const std::vector<Pane> &panes, LayoutType layout_type, int num_masters)
{
    ...
    if (layout_type == LayoutType::TALL)
    {
        if (stack_count > 0)
        {
            ...
            m_renderer->renderRect(default_border_color, first.left() + first.width(), first.top(),
                                   PaneLayout::BORDER_WIDTH, col_height);
        }
        for (size_t i = 0; i + 1 < master_count; ++i)
        {
            const auto &b = panes[i].bounds;
            m_renderer->renderRect(default_border_color, b.left(), b.top() + b.height(), b.width(),
    ...
    else
    {
        if (stack_count > 0)
        {
            ...
            m_renderer->renderRect(default_border_color, first.left(), first.top() + first.height(), row_width,
                                   PaneLayout::BORDER_WIDTH);
        }
        for (size_t i = 0; i + 1 < master_count; ++i)
        {
            const auto &b = panes[i].bounds;
            m_renderer->renderRect(default_border_color, b.left() + b.width(), b.top(), PaneLayout::BORDER_WIDTH,
                                   b.height());
        }
        for (size_t i = master_count; i + 1 < panes.size(); ++i)
        {
