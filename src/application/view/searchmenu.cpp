...
void SearchMenuView::render(const RenderContext &render_context, MenuViewState &state, const domain::Menu &menu,
                            int cursor_column)
{
    ...
    m_renderer->renderRoundedRectWithShadow(getRGBAColor(render_context.ui_options.color_border, default_border_color),
                                            layout.current().x, layout.current().y, layout.current().width,
                                            layout.current().height,
                                            domain::CornerRadius(0, 0, CORNER_RADIUS, CORNER_RADIUS), SHADOW_LENGTH);
    ...
    if (items_size > 0)
    {
        m_renderer->renderRect(
            ...
            layout.current().x, layout.current().y, layout.current().width, 3 * SPACING_MEDIUM + m_input->height(font));
    }
    else
    {
        m_renderer->renderRoundedRect(
            ...
            domain::CornerRadius(0, 0, CORNER_RADIUS, CORNER_RADIUS));
    }
    ...
    m_input->render(m_renderer, render_context, font, menu.getInput(), menu.getInputFace(), cursor_column,
                    state.input_state, input_layout);

    if (menu.hasItems())
    {
        m_renderer->renderRect(getRGBAColor(render_context.ui_options.color_border, default_border_color),
                               layout.current().x - SPACING_MEDIUM, layout.current().y,
                               layout.current().width + SPACING_MEDIUM * 2, 1);
...
domain::MouseMoveResult SearchMenuView::handleMouseMove(float x, float y, const domain::Menu &menu)
{
    if (menu.hasItems())
    {
...
std::optional<int> SearchMenuView::findItemAtPosition(float x, float y, const MenuViewState &state,
...
bool SearchMenuView::handleMouseButton(domain::MouseButtonEvent event, MenuViewState &state, const domain::Menu &menu)
{
    auto item_index = findItemAtPosition(event.x, event.y, state, menu);
    if (item_index.has_value())
    {
        m_mouse_button_observers.notify(item_index.value());
        return true;
    }
    return false;
}

domain::ObserverId SearchMenuView::onMouseButton(std::function<void(int)> callback)
{
    return m_mouse_button_observers.addObserver(std::move(callback));
}

void SearchMenuView::removeObserver(domain::ObserverId id)
{
    m_mouse_button_observers.removeObserver(id);
}

void SearchMenuView::handleMouseScroll(MenuViewState &state, int scroll_amount, const domain::Menu &menu)
{
    if (!menu.hasItems())
        return;
