...
void MovableMenuController::init(MultiStyledMenuView *menu, domain::Window *window)
{
    ...
    m_event_filter_id = window->installEventFilter([this](const domain::WindowEvent &event) -> bool {
        if (!std::holds_alternative<domain::MouseButtonEvent>(event) &&
            !std::holds_alternative<domain::MouseMoveEvent>(event))
        {
            return false;
        }
        ...
        if (std::holds_alternative<domain::MouseButtonEvent>(event))
        {
            ...
            {
                if (button_event.action == domain::MouseButtonAction::PRESS &&
                    (button_event.modifiers & domain::KeyModifier::CONTROL) != 0 &&
                    m_menu->bounds().contains(button_event.x, button_event.y))
                {
                    m_is_dragging = true;
