...
void InputController::init(FocusedClientStack *focused_client_stack, ClientManager *client_manager,
                           domain::Window *window, PaneLayout *pane_layout)
{
    ...
    window->onKeyInput([this](const domain::KeyEvent &event) { onKeyInput(event); });
}
...
void InputController::focusNext()
{
    ...
    auto it = std::find_if(clients.begin(), clients.end(), [current](const auto &c) { return c.get() == current; });
