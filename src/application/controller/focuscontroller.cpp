                           ...
                           PaneLayout *layout_controller, domain::Window *window, MultiStyledMenuView *menu_view)
{
    ...
    m_client_manager->onClientAdded([this](KakouneClient *client) { m_focused_client_stack->focus(client); });

    m_client_manager->onClientRemoved([this](KakouneClient *client) { m_focused_client_stack->remove(client); });

    window->onMouseMove([this](const domain::MouseMoveEvent &event) { onMouseMove(event.x, event.y); });
