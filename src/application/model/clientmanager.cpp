...
KakouneClient *ClientManager::createClient(std::optional<std::string> startup_command,
                                           std::vector<std::string> file_arguments)
{
    int client_id = generateUniqueClientId();
    auto interface =
        std::make_unique<kakoune::JsonRpcKakouneInterface>(*m_session, client_id, startup_command, file_arguments);
