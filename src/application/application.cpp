...
void Application::init(const CliConfig &cli_config, const ApplicationConfig &app_config)
{
    ...
    m_command_controller->init(m_command_interface.get(), m_client_manager.get(), m_kakoune_session.get(), m_window.get(), m_pane_layout.get());
    m_input_controller->init(m_focused_client_stack.get(), m_client_manager.get(), m_window.get());
