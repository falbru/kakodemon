...
ScrolledMenuItems::ScrolledMenuItems(std::shared_ptr<opengl::Font> font, int max_visible_items) : m_font(font), m_max_visible_items(max_visible_items) {
    m_scroll_bar = std::make_unique<ScrollBar>();
}

void ScrolledMenuItems::render(std::shared_ptr<opengl::Renderer> renderer, const KakouneClient &kakoune_client, LayoutManager &layout) {
    ...
    for (int i = m_scroll_offset; i < m_scroll_offset + m_max_visible_items && i < kakoune_client.menu_items.size(); i++)
    {
        ...
        if (i == selected_index)
        {
            renderer->renderRect(
                ...
                items_layout.current().width + SPACING_MEDIUM * 3 + m_scroll_bar->width(), m_font->getLineHeight());
        }
        kakoune::Face item_face = i == selected_index ? kakoune_client.menu_selected_face : kakoune_client.menu_face;

        renderer->renderLine(m_font, item_value, item_face, items_layout.current().x, items_layout.current().y);
        renderer->renderLine(m_font, item_secondary, item_face,
                               items_layout.current().x + items_layout.current().width, items_layout.current().y,
                               core::Alignment::topRight());

        items_layout.sliceTop(m_font->getLineHeight());
    }
