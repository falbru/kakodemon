...
GlyphLines::GlyphLines(const std::vector<GlyphLine> &lines, float line_height)
    : m_lines(lines), m_line_height(line_height)
{
}
        ...
        if (atom_it != atoms.end())
        {
            ...
            if (mode == WrapMode::WORD && last_whitespace_run_index >= 0)
            {
                ...
                for (int i = 0; i < fitted_runs.size(); i++)
                {
                    ...
                    else if (i == last_whitespace_run_index)
                    {
                        std::vector<GlyphMetrics> fitted_part(
                            fitted_runs[i].glyphs.begin(), fitted_runs[i].glyphs.begin() + last_whitespace_glyph_index);
                        std::vector<GlyphMetrics> remainder_part(fitted_runs[i].glyphs.begin() +
                                                                     last_whitespace_glyph_index + 1,
                ...
                for (const auto &run : new_remainder_runs)
                {
                    ...
                    {
                        merged_remainder_runs.back().glyphs.insert(merged_remainder_runs.back().glyphs.end(),
